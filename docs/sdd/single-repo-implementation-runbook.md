...
> 本文是方案 A 的实施与运行手册，位于 scratch 目录，不承担 backlog 状态。所有真实任务状态最终只存在于 GitHub Issues。

## 1. 目标与边界

目标是用两个 GitHub 仓库跑通第一条完整链路：

```text
sdd-platform   # 共享 Factory、Compiler、Schemas、Templates、Workflows
demo-product   # 单产品 monorepo
```
...
1. Factory 通过 GitHub `monorepo-root` Template 创建 `demo-product`。模板自身的初始
   commit 建立 `main`；这是创建仓库所需的 bootstrap，不是 agent 或开发者直接 push。
...
3. Factory 创建 Bootstrap PR，写入产品名、owner 映射、`CODEOWNERS` 和仓库级配置。
4. Bootstrap PR 上运行根目录 CI 和 `PR hygiene`，人工批准后合并。
5. 确认 `CI Gate`、`PR hygiene` 的 check context 已真实产生且为绿，再把它们加入
   ruleset 的 required checks。

GitHub Template API 只复制模板 Git 内容，不复制 labels、rulesets、branch protection、
environments 或 secrets；这些设置必须由 Factory 另行配置。
